...
public interface VagaRepository extends JpaRepository<Vaga, Long> {
	...
	@Query("SELECT NEW Vaga(v.id, v.ativo, v.titulo, v.descricao, v.orcamento, v.prazo, v.statusVaga, v.categoria.id, v.usuarioVaga.id)"
			+ " FROM Vaga v WHERE v.ativo = 1 and v.id = ?1")
	Vaga consultar(Long id);

	@Query("SELECT NEW Vaga(v.id, v.ativo, v.titulo, v.descricao, v.orcamento, v.prazo, v.categoria.id, v.usuarioVaga.id)"
